...
CREATE OR REPLACE VIEW polity__diocese_view AS
    SELECT polity.*, province.id province_id, province.name province_name, province.person_in_charge provine_person_in_charge, province.code province_code
...
CREATE OR REPLACE VIEW polity__deanery_view AS
    SELECT polity.*, diocese.id diocese_id, diocese.name diocese_name, diocese.person_in_charge diocese_person_in_charge, diocese.province_id, diocese.province_name, diocese.provine_person_in_charge, diocese.province_code
...
CREATE OR REPLACE VIEW polity__parish_view AS
    SELECT polity.*, deanery.id deanery_id, deanery.name deanery_name, deanery.person_in_charge deanery_person_in_charge, deanery.diocese_id, deanery.diocese_name, deanery.diocese_person_in_charge, deanery.province_id, deanery.province_name, deanery.provine_person_in_charge, deanery.province_code
